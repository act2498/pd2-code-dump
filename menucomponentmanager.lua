...
function BLTMod:HasModImage()
    -- return true if mod image exists
    return self.json_data and type(self.json_data.image) == "string" and self.json_data.image ~= ""
end
...
function BLTModItem:init(panel, index, ...)
    ...
    -- remove unnecessary child panels and count icons
    local icons = 0
    for _, child in pairs(self._panel:children()) do
        if alive(child) then
            if child.type_name == "Panel" then
                self._panel:remove(child)
            elseif child.type_name == "Bitmap" and child.texture ~= "guis/textures/test_blur_df" then
                icons = icons + 1
            end
        end
    end
    ...
    -- display mod image if it exists
    if self._mod:HasModImage() then
        local mod_image_path = self._mod:GetPath() .. "/" .. self._mod.json_data.image
        local mod_image = self._panel:bitmap({
            texture = mod_image_path,
            w = self._panel:w() - padding * 2,
            h = self._panel:h() - padding * 2,
            layer = 1
        })

        mod_image:set_center_x(self._panel:w() * 0.5)
        mod_image:set_center_y(self._panel:h() * 0.5)
    end
